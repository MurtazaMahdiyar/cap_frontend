...
import AddAlumniModal from "./Forms/AddAlumniModal";
...
const Alumnus = () => {
  ...
  const handleDeleteAlumni = async (data) => {
  ...
  const fetchData = async () => {
    ...
    await fetch(`${baseUrl}/api/v1/students/`, { headers })
      .then((response) => response.json())
      .then((data) => {
        setStudent([...data?.filter((item) => item?.graduated == true)]);
      })
      .catch((error) => {
        console.error("Error:", error);
      });
    ...
    await fetch(`${baseUrl}/api/v1/classes/`, { headers })
      .then((response) => response.json())
      .then((data) =>
        setClasses(data.filter((item) => item?.is_graduated == true))
      )
      .catch((error) => {
        console.error("Error:", error);
      });
  ...
  return (
    <>
      <CustomModal
        ...
        handleConfirm={(data) => {
          setShowCustomModal(false);
          handleDeleteAlumni(data);
        }}
      />
      <AddAlumniModal
        show={showAddAlumniModal}
        facultys={facultys}
        ...
        handleClose={() => setShowAddAlumniModal(false)}
        handleConfirm={(data) => {
